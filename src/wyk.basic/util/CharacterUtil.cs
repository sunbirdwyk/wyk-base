...
namespace wyk.basic
{
    ...
    public class CharacterUtil
    {
        ...
        /// <param name="ch"></param>           
        /// <returns></returns>           
        public static String getPinyinShort(Char ch)
        {
            var rs = getPinyin(ch);
        ...
        /// <returns></returns>   
        public static String getPinyinShort(String source)
        {
            if (String.IsNullOrEmpty(source))
                return String.Empty;
            var sb = new StringBuilder(source.Length + 1);
            var chars = source.ToCharArray();
            for (var i = 0; i < chars.Length; i++)
            {
                sb.Append(getPinyinShort(chars[i]));
            }
            return sb.ToString();
        }
        ...
        /// <param name="source"></param>   
        /// <returns></returns>   
        public static String getPinyinShort(String source, int max)
        {
            if (String.IsNullOrEmpty(source))
                return String.Empty;
            var sb = new StringBuilder(source.Length + 1);
        ...
        public static String getPinyin(String source, string seperator)
        {
            if (String.IsNullOrEmpty(source)) return String.Empty;

            var sb = new StringBuilder(source.Length * (10 + seperator.Length));
            var chs = source.ToCharArray();
            for (var j = 0; j < chs.Length; j++)
            {
                if (j > 0)
                    sb.Append(seperator);
                sb.Append(getPinyin(chs[j]));
            }
            return sb.ToString();
        }
    }
